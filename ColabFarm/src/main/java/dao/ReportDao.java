...
public class ReportDao {
    ...
    public List<FarmerSchemeInfoDto> getFarmersWithMultipleSchemes() {
        ...
        String hql = "SELECT s.farmer.id FROM GovernmentSchemeApplication s GROUP BY s.farmer.id";
        ...
        return farmerIds.stream().map(farmerId -> {
            // Get farmer name
            String nameHql = "SELECT u.name FROM User u WHERE u.id = :farmerId";
            ...
            String schemeHql = "SELECT s.scheme.title FROM GovernmentSchemeApplication s WHERE s.farmer.id = :farmerId";
